 ...
 * @author Lãng
 */
@WebServlet(name = "giamgia", urlPatterns = {"/giamgia"})
public class giamgia extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("id") != null) {
            try {
                ...
                vc = pq.doc_voucher(Md5.getMD5(st));
                if (vc == null) {
                    cart.xoavoucher();
                } else cart.setVouch(vc);
            } catch (SQLException ex) {
                Logger.getLogger(giamgia.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        if (vc != null) {
            response.getWriter().write(String.valueOf(vc.getValue()));
        } else {
