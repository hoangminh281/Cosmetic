...
public class single extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            product p = pq.doc_sp_masp(id);
            ArrayList<product> relateProduct = pq.doc_sp_nhomsplienquan(id,p.getGroupProduct_Id(), 4);
            ArrayList<review> rearr = pq.doc_review(id);
            //-model
            String view = "views/single/v_single.jsp";
            request.setAttribute("view", view);
            request.setAttribute("sanpham", p);
            request.setAttribute("reviews", rearr);
            request.setAttribute("related", relateProduct);
